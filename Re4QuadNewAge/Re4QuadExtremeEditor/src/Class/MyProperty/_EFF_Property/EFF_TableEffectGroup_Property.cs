...
namespace Re4QuadExtremeEditor.src.Class.MyProperty._EFF_Property
{
    ...
    public class EFF_TableEffectGroup_Property : BaseProperty, IInternalID
    {
        ...
        {
            return typeof(EFF_TableEffectGroup_Property);
        }
        ...
        {
            //Line
            ChangePropertyName(nameof(Line), Lang.GetAttributeText(aLang.NewAge_LineArrayDisplayName).Replace("<<Lenght>>", "46"));
        }

        public EFF_TableEffectGroup_Property(EFF_TableEffectGroup_Property prop, bool ForMultiSelection = false)
        {
            PropertyConstructor(prop.InternalID, prop.updateMethods, prop.Methods, ForMultiSelection);
        }

        public EFF_TableEffectGroup_Property(ushort InternalID, UpdateMethods updateMethods, NewAge_EFF_EffectGroup_Methods Methods, bool ForMultiSelection = false) : base()
        {
            PropertyConstructor(InternalID, updateMethods, Methods, ForMultiSelection);
        }

        private void PropertyConstructor(ushort InternalID, UpdateMethods updateMethods, NewAge_EFF_EffectGroup_Methods Methods, bool ForMultiSelection = false)
        {
            this.InternalID = InternalID;
            this.updateMethods = updateMethods;
            this.Methods = Methods;
            groupType = Methods.GetGrouptype();
        ...
        public byte[] Line
        {
            ...
            set
            {
                byte[] _set = new byte[46];
                byte[] insert = value.Take(46).ToArray();
                Line.CopyTo(_set, 0);
        ...
        [CustomCategory(aLang.EFF_Entry_Order_Category)]
        [CustomDisplayName(aLang.EFF_Group_GetEntryCountInGroup_Int_DisplayName)]
        [TypeConverter(typeof(DecNumberTypeConverter))]
        [DecNumber()]
        [DefaultValue(null)]
        [ReadOnly(true)]
        [Browsable(true)]
        [DynamicTypeDescriptor.Id(3, CategoryID_Order)]
        public int EFF_GetEntryCountInGroup
        {
